...
## Building through GitHub only (no local install needed)

This repo includes `.github/workflows/build.yml`, which builds the `.3dsx`
for you using devkitPro's official Docker image. You don't need devkitPro,
a compiler, or anything else installed locally.

1. Create a new **public or private GitHub repo** and push this folder to it
   (via the GitHub web UI's "upload files," GitHub Desktop, or `git`).
2. Go to the repo's **Actions** tab. A "Build 3DSFin" run starts automatically
   on push.
3. When it finishes (green check), open the run and download the
   **3dsfin-3dsx** artifact from the bottom of the page — that's a zip
   containing `3dsfin.3dsx`.
4. Copy `3dsfin.3dsx` to `/3ds/` on your SD card and launch it from the
   Homebrew Launcher, or open it in Citra.

**To get a proper GitHub Release instead of an Actions artifact:** tag a
commit `v0.1.0` (or similar, must start with `v`) and push the tag —
`git tag v0.1.0 && git push origin v0.1.0` — and the workflow will attach
`3dsfin.3dsx` to a Release on that tag automatically. Release assets don't
expire the way Actions artifacts eventually do (90 days by default).

If a build fails, click into the failed step in the Actions log — most
likely cause at this stage is a citro2d API signature mismatch (see
Implementation status below); the error will point at the exact line.

## What devkitPro actually is (optional local setup)

You don't need any of this if you're using the GitHub Actions build above.
